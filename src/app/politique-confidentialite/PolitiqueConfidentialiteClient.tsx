...
export default function PolitiqueConfidentialiteClient({ pageData }: { pageData: PageData }) {
  const { language } = useLanguage();

  const title = pageData?.title?.[language] || pageData?.title?.fr || 'Politique de Confidentialité';
  ...
  const linksTitle: { [key: string]: string } = {
    fr: 'Documents complémentaires',
    en: 'Additional Documents',
    de: 'Ergänzende Dokumente',
    zh: '补充文件',
  };
